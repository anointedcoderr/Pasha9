...
export default function AdminDepositsPage() {
  const { lang } = useLang();
  const [item, setItem] = useState<Deposit | null>(null);
  ...
  return (
    <>
      <PageHeader title="Deposit Management" subtitle={`${mockDeposits.filter((d) => d.status === 'pending').length} pending`} icon={<ArrowDownToLine className="h-5 w-5" />} />
      <DataTable columns={columns} data={mockDeposits} searchPlaceholder="Search by user or TX ID" searchKey="username" />
      <ApprovalModal
        open={open}
        ...
        onConfirm={(note) => console.info(`${action} deposit`, item?.id, note)}
      />
    </>
  );
}
