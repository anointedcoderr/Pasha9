...
import { Sparkles, RefreshCw, Settings, Undo2, AlertCircle, ExternalLink } from 'lucide-react';
            ...
            {(data?.games ?? []).length === 0 ? (
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {data!.games.map((g) => {
                  ...
                  const isPreviewableCode = ['dice', 'mines', 'keno', 'roulette', 'slots', 'crash'].includes(g.gameCode);
                  return (
                    <Card key={g.gameCode} padding="lg">
                      <div className="flex items-start gap-3">
                        {isPreviewableCode ? (
                          <div className="hidden h-20 w-32 shrink-0 overflow-hidden rounded-xl border border-neon/10 sm:block">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="mt-2 text-[11px] text-ink-mid">
                            Test with a small wallet first. Use the preview link to bet 10 BDT before activating publicly.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 flex flex-wrap items-center justify-end gap-2">
                        <Link href={`/games/${g.gameCode}`} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-neon/15 bg-base-panel px-3 text-xs font-semibold text-ink-mid hover:border-neon/30 hover:text-ink-hi">
