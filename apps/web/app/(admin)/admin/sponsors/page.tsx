...
// /admin/sponsors
//
// Mirrors /admin/brand-ambassadors. The Sponsor + BrandAmbassador
// shapes are identical so the page implementation walks the same
// pattern.
...
import { Trophy, Plus, Save, Trash2, Eye, EyeOff, ArrowUp, ArrowDown } from 'lucide-react';
  ...
  const create = async () => {
    ...
    try {
      const r = await fetch('/api/admin/sponsors', {
        ...
        body: JSON.stringify({ nameEn: 'New sponsor', subtitle: '2025/2026' }),
      });
      const j = await r.json();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Sponsorships"
        subtitle="Public sponsor row shown in the footer / about section."
        action={<Button variant="gold" leftIcon={<Plus className="h-4 w-4" />} onClick={create}>Add sponsor</Button>}
      />

      {error ? <Card padding="sm" className="border-l-4 border-rose-400/60"><p className="text-sm text-rose-300">{error}</p></Card> : null}

      <PublicSectionHeader sectionKey="about_sponsors" group="about" />

      {loading ? (
        <Card padding="lg"><p className="text-sm text-ink-mid">Loading...</p></Card>
      ) : rows.length === 0 ? (
        <Card padding="lg"><p className="text-sm text-ink-mid">No sponsors yet. Click Add to create the first one.</p></Card>
      ) : (
...
function SponsorRow({ row, first, last, saving, onPatch, onDelete, onMoveUp, onMoveDown }: {
  row: Row;
  first: boolean;
  last: boolean;
  saving: boolean;
