...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  return withAuth(async () => {
    ...
    const existing = await db.sponsor.findUnique({ where: { id: params.id } });
    if (!existing) return jsonError(404, 'NOT_FOUND');

    const row = await db.sponsor.update({ where: { id: params.id }, data: parsed.data });

    await recordActivity({
      actorId: claims.sub,
      actorRole: claims.role,
      action: 'SPONSOR_PATCH',
      target: params.id,
      meta: { changes: parsed.data },
    });

    return jsonOk({ sponsor: row });
  });
}
